...
let mySwiftString : String = String(myNSString) // NSString을 String으로 변환한 것
...
print(str1 + str2)
print(str1.appending(str2)) //->결과는 동일

//문자열에서 특정 문자 하나를 가지고 올 때
let text = "abc"
//print(text[0]) // 배열처럼 인덱스 접근 불가

//하나만 가져오기
let index = text.index(text.startIndex, offsetBy: 0) // 인덱스 타입으로 변환 후 가져올 수 있다
...
myName2.insert("!", at: myName2.endIndex)//myName2 맨끝에 추가
myName2.insert("@", at: myName2.startIndex)//myName2 맨앞에 추가됨.
...
//정수를 실수로 변환
let myFloat: Float? = Float(myInt!) //언래핑

//래핑클래스
//NSNumber: 모든 숫자 데이터형을 담을 수 있는 클래스(Int, Float, Double, Bool)
//담은 데이터를 필요한 자료형으로 사용할 수 있다
var myInt2:Int = 10
...
let subStr = myNSString2.substring(with: rangeValue)//0에서 3인데 3전까지 의미
