...
//코드 블럭(Code Block) - 코드 뭉치
//코드 뭉치를 변수로 만들고, 매개변수로 전달하고, 실행도 할 수 있다.
//코드블럭을 변수로 만들면 좋은 점: 비동기적인 처리(코드를 읽어들이는 시점과 실행하는 시점)
//통신 - 요청과 응답 처리에 유용하다. UI이벤트 - 클릭이벤트가 생성될 때 실행

//함수형 언어의 특징: 순차적(절차적)언어 -> 클래스 지향언어(자바, C#) -> 함수형 언어(JS, Swift, Kotlin)
...
//값을 전달하는게 아니고, 코드뭉치(블럭, 변수)를 전달해 주는 것
